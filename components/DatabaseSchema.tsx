...
const DatabaseSchema: React.FC = () => {
  ...
  const sqlCode = `-- DIGITAL EMPLOYEE INFRASTRUCTURE (v5.0)
-- 🛠 PERSISTENCE FOR IDENTITY, COMPANY, WHATSAPP & CREDITS

-- 1️⃣ IDENTITY LAYER (Profiles)
-- References auth.users for secure authentication
...
-- 2️⃣ WORKSPACE REGISTRY
-- Links business entities to owners
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-4xl font-black tracking-tight uppercase">Master Ledger v5.0</h2>
            </div>
            <p className="text-slate-400 text-lg max-w-2xl leading-relaxed font-medium italic">
              Corrected schema: **profiles** linked to auth.users, plus workspace and subscription pulse.
            </p>
          </div>
          <button 
            onClick={handleCopy}
      ...
      <div className="bg-white border border-slate-200 rounded-[3rem] overflow-hidden shadow-sm flex flex-col">
        <div className="px-10 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
          <div className="flex items-center gap-4">
            ...
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Identity Protocol v5.0</span>
          </div>
        </div>
